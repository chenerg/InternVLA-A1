...
class WanVAE:
    """WAN VAE wrapper adapted from WoG.

    It keeps the same scale-shift normalization and causal encode pipeline.
    """
    ...
    def __init__(
        ...
        ckpt_relpath="cache/vae_step_411000.pth",
        local_path=None,
        ...
        if local_path is not None:
            vae_pth = Path(local_path).expanduser().resolve()
            if not vae_pth.exists():
                raise FileNotFoundError(f"wan_vae_local_path does not exist: {vae_pth}")
        else:
            vae_pth = hf_hub_download(
                repo_id=repo_id,
    ...
    def encode_tensor(self, video_batch: torch.Tensor, pad_mask: torch.Tensor | None = None) -> torch.Tensor:
        """Encode tensor input.

        Args:
            video_batch: [B, T, C, H, W] in [-1, 1].
            pad_mask: [B, T] with True for padded frames.
        Returns:
            Latents with shape [B, Z, T', H', W'].
        """
        if video_batch.ndim != 5:
            raise ValueError(f"Expected video_batch in [B,T,C,H,W], got shape {tuple(video_batch.shape)}")

        videos = video_batch
        if pad_mask is not None:
            if pad_mask.shape != video_batch.shape[:2]:
                raise ValueError(
                    f"pad_mask shape {tuple(pad_mask.shape)} does not match [B,T]={tuple(video_batch.shape[:2])}"
                )
            videos = videos.clone()
            videos[pad_mask] = 0.0

        videos_list = [rearrange(videos[i], "t c h w -> c t h w") for i in range(videos.shape[0])]
        latents = self.encode(videos_list)
